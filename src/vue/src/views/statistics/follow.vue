<template>
  <div class="block">
    <h2 class="text-left" style="color: #409eff;">关注统计</h2>
    <div class="top">
    ...
    <div>
      <!--  关注统计  -->
      <div id="fansCart" class="resonateCart" :style="{width: '1200px', height: '600px'}"></div>
    </div>
  </div>
</template>
        ...
        methods: {
            ...
            getWeeksFans() {
                ...
                this.api({
                    ...
                    if (this.listresonate.length == 0) {
                        this.$message.error('当前周无排名数据')
                    }
                    this.drawLine();
                })
            },
            getMonthsFans() {
                ...
                this.api({
                    ...
                    if (this.listresonate.length == 0) {
                        this.$message.error('当前月无排名数据')
                    }
                    this.drawLine();
                })
            },
            drawLine() {
                var myChart = this.$echarts.init(document.getElementById('fansCart'))
                myChart.setOption({
                    color: ['#37a2da', '#32c5e9', '#67e0e3', '#9fe6b8', '#ffdb5c', '#749f83', '#ca8622', '#bda29a', '#6e7074', '#546570', '#c4ccd3'],
                    title: {
                        text: this.time + '用户关注排名',
                        subtext: '数据来自益达的数据库'
                    },
                    tooltip: {
                        trigger: 'axis',
